...
- All visual/physics DR flags default to **off** — `PiperXCubesInCupDataGenConfig` inherits them unset. Episodes get sampling-level variation (cube count 1–4, cube/cup placement, robot base pose) plus the always-on TCP action noise, and nothing else.
- **Real-photo background textures (rides `randomize_textures`)**: `dr_texture_paths` (task-sampler config) feeds external images to `TextureRandomizer` as the texture-swap pool. The cubes-in-cup config points it at a 200-image DTD subset (`assets/textures/dtd`, gitignored — wget from the [DTD site](https://www.robots.ox.ac.uk/~vgg/data/dtd/), see `_dtd_texture_paths()`), so the floor, backdrop wall and cup body (the scene's textured geoms; the cup mesh carries cylindrical UVs for this) get a random real photo per roll instead of flat-color jitter — full hue range, messy real-life surfaces. Shown in the `texture_randomization` sampling demo (same randomizer, same roll).
- **Camera mount & FOV noise (always on, both configs)**: per-episode `MjcfCameraConfig` jitter following the upstream recipes — wrist ±4° FOV, ±1.5/0.5/1 cm mount, (8,4,4)° orientation (DROID ZED-mini numbers); the stock config's world-fixed exo ±3° FOV, ±1 cm, ±4° (RBY1-head numbers). The randcam subversion's exo is fully re-sampled instead (`fov_range` 55–80°). See the `camera_noise` sampling demo.
- **Robot base-pose noise (always on)**: `PiperXCubesInCupTaskSampler._sample_and_place_robot` teleports the mocap base per episode — uniform ±3 cm on each of x/y/z, plus ±15° yaw with probability 0.5 (`BASE_POS_NOISE` / `BASE_YAW_NOISE` / `BASE_YAW_PROB`). Cube/cup reach is measured from the noised base; IK/motion-gen and the cuRobo obstacle set are expressed in the live base frame, and the exo "diag view" camera is world-fixed in `cubes_in_cup_scene.xml` (the base-mounted copy in `piper_x.xml` is only used by the older ithor pick config). See the `base_pose_noise` sampling demo.
...
- Robot init-pose noise (Fig 1 **[ON]**) is also inert for PiPER-X: `PiperXRobotConfig.init_qpos_noise_range = None`, so every episode starts at exactly the home qpos (measured across the test dataset). The `init_pose_noise` sampling demo derives a 6-joint range with the Franka recipe (TCP ≤ 10 cm) and shows what enabling it would add.
- Post-hoc re-render (Fig 1 offline renderer DR): the shipped `MultithreadedDomainRandomizationOfflineOpenGLRenderer` is a stub (expects `state_data.npz`/`task_metadata.json` nothing writes; randomizes a nonexistent `model.light_rgba`). The working path is `scripts/viz/piper_x_offline_rerender.py`: rebuild the scene from the episode's `frozen_config` (exact object poses + init qpos), re-execute the recorded clean actions once with action noise off (measured replay: TCP within 5 mm of the recording, same success), then re-render the state tape under fresh lighting/texture draws. Caveat: `SavedEpisode` freezes only the pickup cube + cup, so extra shelf cubes in multi-cube episodes replay parked on the floor.
